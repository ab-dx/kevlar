...
export class WorkflowService {
  ...
  async transitionState(
    ...
    if (!family) {
      throw new NotFoundException(
        `Asset Family ${familyId} not found or access denied.`,
      );
    }

    const currentState = family.status;
  ...
  async rejectAsset(
    ...
    if (!notes) {
      throw new BadRequestException(
        'Rejection requires notes explaining why it failed review.',
      );
    }
    return this.transitionState(
      tenantId,
  ...
  async resetToDraft(tenantId: string, familyId: string, userId: string) {
    return this.transitionState(
      ...
      userId,
      AssetStatus.DRAFT,
      'New version uploaded. FSM reset to DRAFT.',
    );
  }
}
