  ...
  async generateSecureLink(
    ...
    if (!family.activeVersionId)
      throw new BadRequestException(
        'This asset has no active version to share',
      );

    const payload = {
      sub: familyId,
      vid: family.activeVersionId,
      tid: tenantId,
    };
        ...
        async () => {
          ...
          if (!version) {
            throw new NotFoundException(
              'The requested asset version has been deleted or moved.',
            );
          }
          ...
          return await this.minioService.generateDownloadUrl(
            version.minioObjectKey,
          );
        },
      );
    } catch (error) {
